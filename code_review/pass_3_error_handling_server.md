...
**[RESOLVED]** — Fixed in `WebSocketDeckerController.kt` and `MatrixServer.kt`: `CancellationException` is now re-thrown before the general `catch (e: Exception)` handler.
...
**[RESOLVED]** — Fixed in `WebSocketDeckerController.kt`: `KotlinLogging` logger added; exception logged at `error` level in the catch block.
...
**[RESOLVED]** — Fixed in `WebSocketDeckerController.kt`: all hard casts in dispatch helpers replaced with safe `as?` casts and explicit guard returns.
...
**[RESOLVED]** — Fixed in `SessionRegistry.kt`: `session.send(ACTIVE_CONTROLLER)` now called before `turns.setActive(session)`.
...
**[DEFERRED]** — `disconnectedDeckerNames` TTL eviction not implemented; out of scope for this session.
...
**[DEFERRED]** — `register()` send-failure cleanup not implemented; out of scope for this session.
...
**[RESOLVED]** — Fixed in `SessionRegistry.kt`: name validated against `[A-Za-z0-9 _\-]{1,32}` regex, which also rejects blank strings.
