...
**[RESOLVED]** — Four reconnect tests added to `WebSocketServerTest.kt` (lines 178–246): valid token succeeds, wrong token returns `NAME_ALREADY_TAKEN`, missing token for disconnected name returns `NAME_ALREADY_TAKEN`, and invalid characters return `BAD_REQUEST`.
...
**[DEFERRED]** — Action timeout test not added; out of scope for this session.
...
**[DEFERRED]** — `dispatch` exception handler test not added; out of scope for this session.
...
**[DEFERRED]** — Invalid precision string fallback test not added; out of scope for this session.
...
**[DEFERRED]** — File content size guard tests not added; out of scope for this session.
...
**[RESOLVED]** — Both operations removed from `Decker.addHostSystemActions()` entirely (game-logic fix); `availableActions on Host never includes LOCATE_DECKER or SWAP_MEMORY` test added to `DeckerVisibilityTest.kt` (line 252) verifying they are absent.
...
**[DEFERRED]** — Wire-level error path integration tests not added; out of scope for this session.
