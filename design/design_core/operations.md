...
If the target phone has one or more dataline scanners, the decker makes a Computer Skill **Opposed Test** against the scanner's Device Rating. When multiple scanners are present, use only the **highest** Device Rating (not the sum). The Commlink utility reduces the decker's TN on this test (floor 2); the scanner rolls its Device Rating dice at TN = decker's Computer Skill. If the decker's successes exceed the scanner's successes → tap succeeds; otherwise the scanner detects the tap. These scanner tests do **not** affect the decker's RTG security tally — they are resolved entirely outside the normal security-tally system.
