...
**[RESOLVED]** — Fixed in `Game.kt`: `KotlinLogging.logger` added; both `System.err.println` calls replaced with `logger.error(e)`; `buildInitiativeList` wrapped in its own try-catch.
...
**[RESOLVED]** — Fixed in `DeckerNavigationExtensions.kt`: `logonToRtg` now accumulates as `rtg.securityTally + hostTallyDelta`.
...
**[RESOLVED]** — Fixed in `IC.kt`: `applyIcDamage` now called before `resolveSparkyMpcpTest` in `Sparky.action()`.
...
**[RESOLVED]** — Fixed in `IC.kt`: `TarPit.action()` now calls `resolveTarPitMpcpTest` and `removeIc` when `bothCrashed == true`.
...
**[RESOLVED]** — Fixed in `IC.kt`: `context.removeIc(this)` added in `TarBaby.action()` (line 109) and `TarPit.action()` (line 186).
...
**[RESOLVED]** — Fixed in `Game.kt`: `buildInitiativeList` call now wrapped in try-catch with `logger.error`.
...
**[RESOLVED]** — Fixed in `Game.kt`: `KotlinLogging.logger` added; `System.err.println` calls replaced.
